...
name that already carries one. What that buys is a data-integrity property of the bridge's own
state file rather than a guard against a second bridge: this bridge never replaces the identity of a
name it is running and never writes a turn count against a conversation it is not running, and
where two bridges both read the file as absent and both mint, the exclusive create refuses the
loser and tells its caller. It does not reach the case section 5 observes as inferred, two sessions
in two directories whose scope key has collapsed to a single directory: a bridge holding no record
of the name adopts what the file says, which is byte-identical to the restart-resume the plan
requires to work, so the residual hazard is named rather than closed. Where the file is present the
write takes no lock either, so a write whose read lands inside another write's parse-to-rename
interval lays over it with neither caller told.

That survivor is recorded under Assumptions with its reversal, because
...
- **The three dependency advisories are dispositioned here, with the lockfile change landing in this
  section's own diff.** `npm audit` reports `fast-uri` (high), `qs` (moderate) and `hono` (moderate),
  all three transitive through `@modelcontextprotocol/sdk`: the first through its schema validator, the
  second through Express, and the third through the SDK's HTTP server transport, which reaches `hono`
  both directly and through `@hono/node-server`. Reachability from the bridge is low and is not the
  reason this is owed: the bridge runs the stdio transport, so Express, `qs` and the whole HTTP server
  path including `hono` are off its path, and the validator is reached only where the SDK validates a
  schema with it. What makes it this section's is that a lockfile change reddens
  ...
  in review. The disposition is either the upgrade that clears all three or a recorded decision to accept
...
- assumed 2026-09-08 (the Worker's recommendation, accepted by the CHANNELS Expert seat as
  ...
  different identifier over a name already carrying one refuses; what this buys is a data-integrity
  property of the bridge's own state file and not a guard against a second bridge, so it does not
  cover the scope-key collapse section 5 observes as inferred, which is two sessions in two
  directories rather than the operator error the ruling assumes away: a bridge holding no record of
  the name adopts what the file says, and the residual hazard is named rather than closed.
  Reversal: delete about a dozen lines and their test, and
...
Completed: 2. Bridge core
Implemented By: implementer-opus, escalated to implementer-fable once the tier-escalation ladder fired, with every round from that point running at fable; twenty-one fix rounds in all, one of which (round 20) was stopped on the wedge hallmark with its fix unstarted and re-dispatched as round 21.
Metrics: review rounds 20, closed major-closed; NEEDS_CONTEXT 1; escalations 1; consults 3
Decisions / Surprises: The section's shape was decided by the operator's four rulings, recorded at 67db586 and confirmed to this session directly. The ownership lease that rounds 8 through 20 had grown one repair per round defends a configuration the operator will not run, so it was removed by subtraction rather than finished; the review loop was capped at round 20 plus one subtraction round and one bounded review round on that delta; the section's code commits at the cap rather than at first green; and prose-level Minors batch into one round rather than being fixed per round, which binds every later section. What survives the lease's removal is a single compare-and-set on the conversation identifier, a value the worker runtime mints and this code never invents, plus a purely local record of the names this process has prompted, carrying no process identifier and reading nothing about any other process. Round 21 returned a question rather than a diff, and the question was sound: removing the lease removed the only thing distinguishing a name this process is running from a name it merely read out of the shared file at construction, and the surviving compare-and-set compares exactly that. It was ruled from the spec rather than escalated, on the plan's own scope line. Three plan-doc passages are corrected in this changeset, named here as the corrections they are: the Approach paragraph on one bridge per scope and session name, which said the survivor "refuses rather than runs two runtimes on one worker log"; the 2026-09-08 Assumptions entry, which said it "covers the scope-key collapse"; and the qualifying clause both now carry, since the wording that propagated the overclaim was this session's own, at `.kit/r21-ruling.md` lines 68 to 69. All three were stronger than the code can be, and the reason is structural rather than a missing guard: a bridge holding no record of a name that adopts the file's record is byte-identical to the restart-resume the plan requires to work, so no version of this guard can tell the hazard from the feature. What the compare-and-set does buy is stated at exactly its strength in the code, in `bridge/README.md` and now in the plan: this bridge never replaces the identity of a name it is running, never writes a turn count against a conversation it is not running, and where two bridges both read the file as absent and both mint, the exclusive create refuses the loser and tells its caller; where the file is present the write takes no lock, so a write whose read lands inside another write's parse-to-rename interval lays over it with neither caller told. The residual hazard is named rather than closed, on both surfaces. Two instrument facts cost real time and are worth carrying. A dispatched agent's `.output` artifact is not a growth instrument here: its non-synthetic assistant-line count ran backwards across two readings eight minutes apart while the file's own mtime stayed frozen, matching the byte-identical stale slices two blocking calls returned, so file mtimes are the sound growth instrument for a dispatch on this box. And a process poll narrowed to one runtime answers a narrower question than the label put on it: a poll grepping for `testhost` and `dotnet` truly reports no stray .NET test processes and says nothing whatever about whether the box is quiet, which is the trap operator memory neo-claude-parallelism-is-measured-never-named records. A third was confirmed here: Node prints its test summary lines behind a leading multi-byte character, so a one-character-anchored count pattern returns nothing on a green run and reads exactly like a suite that produced no summary. Two items are recorded here rather than acted on, both by choice. Guard 4's second-pass EEXIST refusal is covered by reading and not by a run, because it is not deterministically stageable on this platform. And the superseded-then-resume pin proves the bridge's own bookkeeping rather than the real runtime's acceptance of a resumed conversation id, because the fake accepts any id; the round's new turn-count pin runs the fake under a stored id it never minted and the turn completes, which confirms the fake's behaviour on that path and is a section 5 observation to sit beside the existing unknown-id note. One item was routed outward: this session's own read-only `npm audit` reports three advisories rather than the two section 4's criterion named, `fast-uri` (high), `qs` (moderate) and `hono` (moderate), all three transitive through `@modelcontextprotocol/sdk`, and the criterion is updated in this changeset to name `hono` and its path through the SDK's HTTP server transport. No `npm audit fix` was run, since a lockfile change reddens suites outside its own diff and the disposition is section 4's. One further item was caught at the close rather than in a round, and it is recorded as the approval drift it is: `bridge/protocol.ts` imports `bridge/reader-class.ts` and `bridge/protocol.test.ts` imports `bridge/fixtures/claude-code-reader.ts`, and neither of those two files is named in section 2's `Files in scope:` line nor present in HEAD, section 1's close having never committed them. A push without them would have shipped a tree that cannot type-check on a fresh clone, so both ship in this changeset. The check that found it resolves every relative import under `bridge/` against the prospective commit set rather than against a list of file names, and it returned zero missing targets under a control that withheld a known-present target and was seen to discriminate.
Assumptions: assumed 2026-09-08 (the Worker's recommendation, accepted by the CHANNELS Expert seat as low-blast and reversible rather than put to the operator): after the lease's removal the state file keeps one compare-and-set on the conversation identifier; the entry is corrected in this changeset to state that this buys a data-integrity property of the bridge's own state file and not a guard against a second bridge, so it does not cover the scope-key collapse section 5 observes as inferred. Reversal unchanged: delete about a dozen lines and their test, and name the residual hazard in `bridge/README.md` and this Chapter.
Review Findings: review: code pair at fable, Agent tool (adversarial at frontmatter effort low, blind at low); review: security at fable, Agent tool (medium); consults 3 across the section, at rounds 6, 17 and 19, each at fable. The final bounded round returned CHANGES_REQUIRED and CONCERNS from all three lenses with no tree delta across the round bracket, and three Majors, all in surviving or newly written code rather than in code the subtraction removed, so all three were fixed once with no further round per the operator's second ruling. Majors addressed: (1) the reader's admission predicate and the compare-and-set's had diverged, the reader requiring a workspace that passes the path guard and the write testing only the session id, so an entry with a sound id and a network-path workspace was dropped by the reader and read as another conversation by the writer, which produces a permanent mint-refuse-repeat livelock on that name; found independently by the security and blind lenses, the blind lens with no spec, no section name and no ruling, and its reachable input was already staged as an ordinary fixture in the suite. Fixed in the code rather than in the prose, because `bridge/README.md` already stated the correct rule and carried its own reason, so weakening the doc would have deleted the argument for the behaviour we want: one exported predicate now serves both readers, and the write's admitted set is a strict subset of the old id-only test's, so nothing is newly refused and the newly-admitted shape is an entry no bridge on this machine can write. (2) the missing-workspace refusal preceded the unreadable-map refusal, so a name with a record on disk during an unreadable map was told it "is new, so dsh_prompt needs its cwd", false in both halves, and the round had coded around it in a test rather than fixing it; the check now sits above the recall and the workaround and its comment are gone. (3) the unconditional claim that the losing write refuses and its caller is told, in both the state-file doc comment and the README, whose wording came from this session's own ruling file; fixed in the prose rather than by adding a lock, because a lock around the state write re-imports the multi-writer premise the operator's first ruling struck. Minors addressed, batched per the fourth ruling: the fractional-and-NaN gap in both protocol bounds, now guarded in the form that is also true of NaN, with literal pins in both directions; the turn count now taking the runtime's number in both directions rather than only upward, with a new fake switch to stage a runtime number behind the file's count; the diagnostic-line bound now counting only record-scoped lines, so a later file-level failure is still said; the stale filesystem-call wording in three places; and the unconditional `dsh_status` claim in the README, now qualified by whether this bridge has prompted the name. Majors justified rather than fixed: none. Findings discarded with reason: six sidecar items were checked against the code and all six fell, including a claimed residual existence-check use (comments and out-of-scope legitimate uses), claimed lease residue at three test sites (pre-fix line numbers; the only residue is a deliberately kept foreign-shape fixture), and a claimed failing index suite at 11 of 12 (read from a superseded pre-correction marker file, where the current marker is exit 0 at 12 of 12). Every finding was verified against the code rather than accepted from its report, and the one real discrepancy in the final round's own report, a file mtime postdating the green it claimed, was settled by content rather than inference: a byte comparison against the pre-fix copy plus three positive markers inverting cleanly.
Stamps: adjudicated 22, stamped 3 (neo-claude-parallelism-is-measured-never-named, unstamped-lists-peer-session-reads, verify-a-splice-by-diffing-not-by-grepping). The other nineteen candidates are peer-session reads by subject, coordinator board clearance, delegation grants and store sync among them, which this session's work never touched; skipped on the rule operator memory unstamped-lists-peer-session-reads states, that a stamp asserts the record steered this session's own work.
Gate: Targeted lane at the section close, 2026-09-08T23:00Z on NEO-CLAUDE with a live foreign heavy-process claim present earlier in the round and released before the whole gate: type check exit 0, then per-suite runs, harness 65 tests 65 pass 0 fail exit 0, protocol 23/23/0 exit 0, index 12/12/0 exit 0, log 18/18/0 exit 0, fake-dsh 2/2/0 exit 0, env 3/3/0 exit 0, every exit code read from the run itself and the count-reading instrument controlled against a log known to carry a failure, which spoke (tests 1, pass 0, fail 1). Whole gate beside it because this section's push lands on main, a trunk consumers install from with no CI gating the merge, 2026-09-08T23:07:38Z to 23:09:13Z on NEO-CLAUDE: lint exit 0, test exit 0, tests 1705, pass 1704, fail 0, skipped 1, 95 seconds. Delta against the baseline recorded on that same lane, the 2026-09-07T19:59:14Z whole-gate run on this checkout (tests 1609, pass 1608, fail 0, skipped 1, 147.6 seconds): plus 96 tests and plus 96 passes from rounds 20 and 21's pins, failures 0 both sides, skipped 1 both sides, so no regressions. Contention lane beside both, which is the second trigger rather than the same one: three independent instruments agree the box has 4 logical processors, average processor load read 96 percent immediately before the whole gate and 100 percent immediately after, with 22 node-family processes present and no foreign test runner in either poll; the heavy-process slot was held by this session for the duration under the claim protocol, written only after a read showed it absent and released on a match against this session's own id. The gate ran faster than its baseline despite that contention, so the duration carries no regression either.
Next: 3. Record writer
Commit Model: Commit-and-Push
Delta: reading taken 2026-09-08T23:10:14Z on NEO-CLAUDE, under the contention named on the Gate line above.
