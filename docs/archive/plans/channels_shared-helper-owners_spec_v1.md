...
None open. `channels_board-worker-queues_spec_v1.md` (complete, beside this plan in the archive) is the plan whose
...
Implemented By: main session, with qa-verifier, performance-reviewer, security-reviewer, adversarial-reviewer, scope-adjudicator, implementer-sonnet (fix round 1) and docs-curator
Metrics: review rounds 2, closed major-closed; provenance 1 spec-traceable, 0 fix-introduced, 0 new-requirement, rulings (0 refused, 4 declared, 0 asked); advisory: 5 findings, 0 fixed, 2 deferred, 2 refused, 1 routed to the Minor pass and refused there; NEEDS_CONTEXT 0; escalations none; consults 0
Recap: Goal: "Each small mechanism the broker has copied by hand across its modules lives in one place that every caller imports, and the one copy that is wrong today is fixed by the move. Four families are copied now. The rate-limited repeat logger exists as eight copies. The standing cards' binding module exists as three. The size-capped file read exists as four. The non-finite modification-time clamp exists as two. The usage cache's copy of the capped read performs a single read with no loop, so a short read hands the parser a prefix of the file under the name of the whole, which the parser refuses as malformed and the usage card reads as unavailable for that poll. The codebase's own precedent set three copies as the point to extract, and every family but the clamp is past it. When this is done each family has one owner, the usage cache's read loops like its siblings, and no caller's observable behaviour changes otherwise. The four backlog entries left the active list when the operator approved the candidate for planning, and sit in `docs/archive/backlog-2026-Q3.md` under "Planned 2026-09-22"."; What the tree does now: the broker's helper that reads a file up to a size limit, its helper that stops one error from flooding the log, the code that remembers which Discord thread each standing card lives in, the pattern that checks a Discord ID, and the guard that keeps a bad file timestamp out of the board's sorting each exist once, and every part of the broker that needs one imports it. The usage card's file read now keeps reading until it has the whole file, so a disk that hands the file over in pieces no longer makes the card show "unavailable" for that poll. Every log line reads exactly as before, and each module's own tests now check that its lines carry its own name; Refinements during the run: the capped read keeps the old `CappedRead` type name as an alias in the two modules that exported it (Chapter 1's assumption); each repeat-log surface exports its line texts as a constant so the tests can pin them (Chapter 2); the card binding's save takes no label, since it writes no log line (Chapter 3, recorded as a deviation); a third copy of the markdown-suffix constant in the board card was folded onto the shared one, and the README rule is shared as a function rather than a constant (Chapter 4); the finishing pass added a per-module prefix test for five repeat-log surfaces, made the surface constants read-only and renamed the board card's private stem helper; no spec amendment and no operator ruling mid-run; Operator-pending: write the project's threat model (already on `docs/backlog.md`, handoff 2026-09-21).
Decisions / Surprises:
- Base ref f0f6fbc, the merge-base of `shared-helper-owners` with `origin/main`. The changeset listing against it held one path outside every section's `Files in scope:` and the bookkeeping set, `docs/backlog.md`, which this plan's own backlog entries explain.
- Fix round 1 (finishing): the adversarial Major that five of the eight repeat-log surfaces had no test proving the module hands the core its own surface is spec-traceable to the Intent's "keeps its exact text per surface"; fixed by one prefix assertion per surface through the module's own factory (two tightened, three added), each observed red on a scratch copy with the prefix changed; adds no mechanism; tests only.
- Minor pass (finishing): `RepeatLogSurface` fields are `readonly`, so no importer can change a surface's window or cap at runtime; the board card's private `planStem` is `drawnStem`, since `plans.ts` exports a `planStem` with another contract; one test comment corrected. Refused: splitting the thin callers' inline `type` imports (`registry.ts`, `routing/outbound.ts` and `tail.ts` use the same form); inverting `statPlanFile`'s default (Chapter 4's decision, and the one caller over foreign-written names opts in).
- The goal read's brief quoted the Intent's frame but not its Rulings paragraph, so two items it ruled built-but-unasked (the injectable reader, the state-before-log order) are in fact the architect's rulings; they are declared here either way.
- Drift: two `deviation` items, no `mistake`. D1: `saveCardBinding` takes no label, against the spec's "one load and one save taking a label" (Chapter 3). D2: `plans.ts` shares `isReadmeStem` rather than a README constant, keeps `WHITESPACE_RUN` private, and `MARKDOWN_SUFFIX` goes to the card rather than the queue reader (Chapter 4). Both rest on the spec and the code, with no pre-change claim to read. `docs/architecture.md` gained "Helpers with one owner" and three corrected passages.
- The backlog lost two entries this plan had added, the threat model and the `npm audit` findings, because standing entries from earlier plans already carry both (curator hygiene item H3).
- `origin/main` was merged into the branch before the archive (089d3c0), bringing PR #25's archived tail-until-flake plan; no conflict.
Assumptions:
- assumed 2026-09-22 (the main session, section 1): where a module exported its own `CappedRead`, it keeps that name as a type alias of the shared one rather than dropping the export, so no module's public surface changes; reversal: delete two alias lines and point the option types at the shared type.
- declared 2026-09-23 (goal read): the shared capped read takes an optional reader in place of `readSync`, a test seam no production caller passes.
- declared 2026-09-23 (goal read): all eight repeat loggers refresh their window before writing, the inbox judge's order; observable only when the log function itself throws.
- declared 2026-09-23 (goal read): `SNOWFLAKE` has one owner in `broker/security/senders.ts`, a fifth family the Goal's list does not name; the pattern is byte-identical.
- declared 2026-09-23 (goal read): the plan-file helpers are exported from `broker/board/plans.ts`, with `statPlanFile` taking a regular-file flag the queue reader turns on.
Review Findings: review: performance + security + adversarial at fable, Workflow (effort high), resolved models 29, 38 and 36 turns all `claude-fable-5-1`; then adversarial over the fix delta at fable, 34 turns `claude-fable-5-1`. Critical 0. Major 1, fixed (fix round 1). Minors: 3 fixed in the Minor pass, 2 refused, 1 routed to the index refresh; advisory: performance CLEAR (sweep copy deferred as pre-move, negative read count refused), security CLEAR with `threat model: absent` (`npm audit` and `SNOWFLAKE` comment deferred to their backlog entries, `statPlanFile` default refused). Goal read: scope-adjudicator at fable, RULED, BUILT-BUT-UNASKED 4 (all accept-and-declare, recorded above), ASKED-BUT-UNBUILT 0.
Stamps: adjudicated 1 listed; stamped `workflow-parallel-caps-at-two` (operator tier, applied to the review round's windows); skipped `subagent-can-report-a-documented-past-injection-as-a-live-one` (read by the fix implementer, which found nothing to check).
Gate: whole gate `npm run lint` then `npm test` over the final tree (origin/main merged at 089d3c0, plan archived, indexes refreshed) at 2026-09-23T00:39:15Z-00:40:06Z, SCOTT-CLAUDE, box polled CLEAR before and after: lint exit 0; tests 2035 / pass 2034 / fail 0 / cancelled 0 / skipped 1, exit 0, 50 s. Baseline, finishing step 1 at 9bed809: 2029 / 2028 / 0 / 1 skipped, exit 0, 42 s. Delta +6: this pass's 3 added prefix tests (2 more tightened), and +3 from the merged tail-until-flake plan (4 added, 1 retired); the skip is the pre-existing POSIX-only token-file test on both runs. Contention lane: this repository defines none. Over the whole effort: tests added 29, retired 0, edited 2. Spawning tests added 0.
Next: none; the plan is Complete.
Commit Model: Branch-and-PR
Delta: kit-size measures no corpus in this repository (Chapter 4's reading).
