...
export type ChannelPins = {
  ...
  /**
   * Deletes a message in the host's configured channel, where the pin notices land. Its caller is
   * the gateway, which spends it on the system message Discord writes when this bot pins: deleting
   * one's own message needs no Manage Messages grant, so it works on the same permissions the pins
   * do.
   *
   * The notice a rename writes into a thread is not deletable this way. An application asking for
   * it is answered 403 with error 50021, whatever it holds, so nothing here asks.
   */
