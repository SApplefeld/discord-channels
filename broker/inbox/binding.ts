...
//
// Written the way the board card's binding beside it is: a versioned snapshot to a sibling temp file
...
//
// Its own file rather than a record inside another surface's: every standing card is independent,
// each built under its own knob, and a broker running one of them writes and reads nothing on
// another's account.
...
export type LoadInboxBindingOptions = {
...
export function loadInboxBinding(
  ...
  try {
    ...
    log(
      `broker: cannot read the inbox card binding at ${file}, starting with none: ${String(error)}`,
    );
    return null;
  }
  ...
  try {
    ...
    log(
      `broker: the inbox card binding at ${file} is not valid JSON, starting with none: ` +
        String(error),
    );
    return null;
  }
  ...
  if (!isRecord(parsed) || parsed.version !== FORMAT_VERSION || !isRecord(parsed.binding)) {
    log(
      `broker: the inbox card binding at ${file} is not a snapshot of this format, ` +
        `starting with none`,
    );
    return null;
  }
  ...
  if (
    ...
    log(`broker: the inbox card binding at ${file} is malformed, starting with none`);
    return null;
  }
  ...
  if (!SNOWFLAKE.test(binding.messageId) || !threadIdValid) {
    log(
      `broker: the inbox card binding at ${file} names something that is not a Discord ` +
