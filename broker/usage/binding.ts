...
//
// Written the way the session bindings beside it are: a versioned snapshot to a sibling temp file
...
export type LoadUsageBindingOptions = {
...
export function loadUsageBinding(
  ...
  try {
    ...
    log(
      `broker: cannot read the usage card binding at ${file}, starting with none: ${String(error)}`,
    );
    return null;
  }
  ...
  try {
    ...
    log(
      `broker: the usage card binding at ${file} is not valid JSON, starting with none: ` +
        String(error),
    );
    return null;
  }
  ...
  if (!isRecord(parsed) || parsed.version !== FORMAT_VERSION || !isRecord(parsed.binding)) {
    log(
      `broker: the usage card binding at ${file} is not a snapshot of this format, ` +
        `starting with none`,
    );
    return null;
  }
  ...
  if (
    ...
    log(`broker: the usage card binding at ${file} is malformed, starting with none`);
    return null;
  }
  ...
  if (!SNOWFLAKE.test(binding.messageId) || !threadIdValid) {
    log(
      `broker: the usage card binding at ${file} names something that is not a Discord ` +
