...
/**
 * Runs a PowerShell script file and resolves with its exit status and streams.
 *
 * Spawned rather than spawnSync'd because PowerShell startup is this file's whole cost, and a
 * synchronous spawn blocks the event loop, which leaves the concurrency the suite below declares
 * with no await point to use.
 *
 * Only the two helpers below route through here. The ACL tests further down still call `spawnSync`
 * directly, about a dozen times, and each of those does block the event loop for a full PowerShell
 * startup, so the declared concurrency is unavailable to whatever else is in flight during those
 * windows. That is a deliberate stopping point rather than an oversight: converting the two helpers
 * alone already put this file below the slowest file in the suite, which is the only thing the wall
 * clock responds to, so the rest of the conversion would have been risk bought for nothing. The
 * short `icacls.exe` and `cmd.exe` setup calls stay synchronous on their own merits, being tens of
 * milliseconds against PowerShell's several hundred.
 */
function runPowerShell(scriptPath: string): Promise<{ status: number | null; stdout: string; stderr: string }> {
  return new Promise((resolve) => {
    const child = spawn("powershell", ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", scriptPath]);
    // A synchronous spawn handed the child an already-closed stdin, so a script that read from it
    // failed at once. A spawned child is given a live pipe and would wait on it forever, and the
    // runner applies no per-test timeout, so the whole file would hang rather than fail.
    child.stdin.end();
    let stdout = "";
    let stderr = "";
    ...
    // A child process 'error' event with no listener is rethrown as an uncaught exception, which
    // would take the whole file down instead of failing the one test. A synchronous spawn reported a
    // failure to launch as a null status, and the callers here already assert on the status, so this
    // resolves the same shape and the failure stays a readable assertion.
 ...
 *
 * This asserts on the child's exit status itself, so a call site that forgets to await it turns a
 * failed assertion into an unhandled rejection that no test owns. Every call site awaits.
 */
async function runFunctions<T>(body: string, directory: string): Promise<T> {
...
async function runFunctionsExpectingFailure(
  body: string,
  directory: string,
...
// These launch real PowerShell processes against real temp directories, which is this file's whole
// cost. They share nothing: each builds its own temp directory and removes it in t.after(), none
// touches process.env or the working directory. The bound is a number rather than `true` because
// the runner is already running other files alongside this one.
describe("Install-Functions", { concurrency: 8 }, () => {
  it("Get-SubstitutedFragment rewrites only the SessionStart path", async (t) => {
    const dir = tmpDir();
    ...
    const fragment = await runFunctions<Fragment>(
      [
  ...
  it("Merge-ChannelSettingsFile creates a fresh settings file with all five events", async (t) => {
    const dir = tmpDir();
    t.after(() => rmSync(dir, { recursive: true, force: true }));
    const settingsPath = path.join(dir, "settings.json");

    type Settings = { hooks: Record<string, unknown[]> };
    const merged = await runFunctions<Settings>(
      [
  ...
  it("Merge-ChannelSettingsFile preserves unrelated hooks and settings", async (t) => {
    const dir = tmpDir();
    t.after(() => rmSync(dir, { recursive: true, force: true }));
    const settingsPath = path.join(dir, "settings.json");
    writeFileSync(
  ...
  it("running the merge twice does not duplicate the project's own hook entries", async (t) => {
    const dir = tmpDir();
    t.after(() => rmSync(dir, { recursive: true, force: true }));
    const settingsPath = path.join(dir, "settings.json");
  ...
  it("Assert-ValidChannelFragment accepts the fragment this repository ships", async (t) => {
    ...
    const result = await runFunctions<{ accepted: boolean }>(
      [
        `$fragment = Get-SubstitutedFragment -FragmentPath "${FRAGMENT_PATH}" -SessionStartScriptPath "C:\\repo\\hooks\\session-start.ps1"`,
  ...
  it("re-running the merge with a moved checkout's path replaces the stale entry, not beside it", async (t) => {
    const dir = tmpDir();
    t.after(() => rmSync(dir, { recursive: true, force: true }));
    const settingsPath = path.join(dir, "settings.json");

    type Settings = { hooks: { SessionStart: [{ hooks: [{ command: string }] }] } };
    const result = await runFunctions<Settings>(
      [
  ...
  it("Test-IsChannelHookEntry does not delete an unrelated tool's same-named hook", async (t) => {
    const dir = tmpDir();
    t.after(() => rmSync(dir, { recursive: true, force: true }));
    const settingsPath = path.join(dir, "settings.json");
  ...
  it("Merge-ChannelSettingsFile refuses a fragment with an event outside the allowed set", async (t) => {
    const dir = tmpDir();
    t.after(() => rmSync(dir, { recursive: true, force: true }));
    const settingsPath = path.join(dir, "settings.json");

    const { status, stderr } = await runFunctionsExpectingFailure(
      [
        `$fragment = Get-SubstitutedFragment -FragmentPath "${FRAGMENT_PATH}" -SessionStartScriptPath "C:\\repo\\hooks\\session-start.ps1"`,
  ...
  it("Merge-ChannelSettingsFile installs the relay's reply rule beside the operator's own", async (t) => {
    ...
    const merged = await runFunctions<Settings>(script, dir);
    assert.deepEqual(
    ...
    const again = await runFunctions<Settings>(script, dir);
  ...
  it("Merge-ChannelSettingsFile refuses a permission rule outside the installer's own list", async (t) => {
  ...
  it("Merge-ChannelSettingsFile refuses a fragment whose SessionStart command is not the substituted one", async (t) => {
    const dir = tmpDir();
    t.after(() => rmSync(dir, { recursive: true, force: true }));
    const settingsPath = path.join(dir, "settings.json");
  ...
  it("Merge-ChannelSettingsFile refuses a fragment declaring an unrecognized hook type", async (t) => {
    const dir = tmpDir();
    t.after(() => rmSync(dir, { recursive: true, force: true }));
    const settingsPath = path.join(dir, "settings.json");
  ...
  for (const event of ["UserPromptSubmit", "PreToolUse", "PostToolUse", "Stop"] as const) {
    it(`Merge-ChannelSettingsFile refuses a command hook declared under ${event}`, async (t) => {
  ...
  it("Merge-ChannelSettingsFile refuses an http hook posting anywhere but this project's own routes", async (t) => {
  ...
  it("Merge-ChannelSettingsFile refuses an http hook setting a header this project does not set", async (t) => {
    const dir = tmpDir();
    t.after(() => rmSync(dir, { recursive: true, force: true }));
    const settingsPath = path.join(dir, "settings.json");
  ...
  it("Merge-ChannelSettingsFile refuses an http hook authorizing an environment variable of its own", async (t) => {
  ...
  it("Merge-ChannelSettingsFile admits the mirror switch header and env var on a mirror-route hook", async (t) => {
    ...
    const result = await runFunctions<{ merged: boolean }>(
      [
        `$fragment = Get-SubstitutedFragment -FragmentPath "${FRAGMENT_PATH}" -SessionStartScriptPath "C:\\repo\\hooks\\session-start.ps1"`,
  ...
  it("Merge-ChannelSettingsFile refuses the mirror switch header on a liveness hook", async (t) => {
  ...
  it("Merge-ChannelSettingsFile refuses a PreToolUse entry whose matcher is not exactly AskUserQuestion", async (t) => {
    ...
    const widened = await runFunctionsExpectingFailure(
      [
        `$fragment = Get-SubstitutedFragment -FragmentPath "${FRAGMENT_PATH}" -SessionStartScriptPath "C:\\repo\\hooks\\session-start.ps1"`,
    ...
    const missing = await runFunctionsExpectingFailure(
      [
        `$fragment = Get-SubstitutedFragment -FragmentPath "${FRAGMENT_PATH}" -SessionStartScriptPath "C:\\repo\\hooks\\session-start.ps1"`,
  ...
  it("Merge-ChannelSettingsFile refuses a mirror switch header that does not interpolate the variable", async (t) => {
  ...
  it("Merge-ChannelSettingsFile refuses http hooks that do not agree on one port", async (t) => {
  ...
  it("Merge-ChannelSettingsFile prunes old backups beyond the retention count", async (t) => {
    const dir = tmpDir();
    t.after(() => rmSync(dir, { recursive: true, force: true }));
    const settingsPath = path.join(dir, "settings.json");
    writeFileSync(settingsPath, JSON.stringify({}), "utf8");

    await runFunctions<{ hooks: unknown }>(
      [
        `$fragment = Get-SubstitutedFragment -FragmentPath "${FRAGMENT_PATH}" -SessionStartScriptPath "C:\\repo\\hooks\\session-start.ps1"`,
  ...
  it("Get-ChannelStateRoot resolves under the given LOCALAPPDATA", async (t) => {
    const dir = tmpDir();
    t.after(() => rmSync(dir, { recursive: true, force: true }));

    const result = await runFunctions<{ root: string }>(
      [
  ...
  it("Get-ChannelStateRoot throws when LOCALAPPDATA is unset", async (t) => {
    const dir = tmpDir();
    t.after(() => rmSync(dir, { recursive: true, force: true }));
  ...
  it("Set-ChannelEnvFile and Get-ChannelEnvFile round-trip", async (t) => {
    const dir = tmpDir();
    t.after(() => rmSync(dir, { recursive: true, force: true }));
    const envPath = path.join(dir, "broker.env");

    const result = await runFunctions<Record<string, string>>(
      [
  ...
  it("a reinstall preserves an allowlisted knob the operator set by hand", async (t) => {
  ...
  it("Set-ChannelBrokerEnvironment applies only allowlisted keys", async (t) => {
    const dir = tmpDir();
    t.after(() => rmSync(dir, { recursive: true, force: true }));
    const envPath = path.join(dir, "broker.env");
  ...
  it("every knob broker/config.ts reads is on the installer's env allowlist", async () => {
    ...
    // silently. The board card's five and the inbox's four are named here so that placement stays
    // deliberate.
    for (const knob of [
      "CHANNEL_BOARD_CARD",
  ...
  it("Register-BrokerTask's function throws before touching the Task Scheduler when not elevated", async (t) => {
    const dir = tmpDir();
    t.after(() => rmSync(dir, { recursive: true, force: true }));
    ...
    writeFileSync(
      ...
      [
        `. "${registerScript}"`,
        `Register-BrokerScheduledTask -TaskName "ProbeTaskUnelevated" -ScriptPath "C:\\does\\not\\matter.ps1" -User "TESTDOMAIN\\TestUser" -IsElevated:$false`,
      ].join("\n"),
      "utf8",
    );
  ...
  it("Register-BrokerTask's function builds a task definition without registering it", async (t) => {
    const dir = tmpDir();
    t.after(() => rmSync(dir, { recursive: true, force: true }));
    const registerScript = path.join(path.dirname(FUNCTIONS_PATH), "Register-BrokerTask.ps1");
    ...
    const definition = await runFunctions<Definition>(
      [
        `. "${registerScript}"`,
    ...
    //
    // The name is this test's alone, and must stay that way. This reads real machine-wide state
    // while sibling tests run concurrently, so a second test registering the same name would decide
    // this assertion. The unelevated test above deliberately uses "ProbeTaskUnelevated".
