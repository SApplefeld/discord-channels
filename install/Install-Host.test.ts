 ...
 *
 * This is a plain function returning a promise rather than an `async` one, and deliberately so: the
 * two fencing guards throw synchronously, which is what lets the guard test hold them with
 * `assert.throws`. Marking this `async` would turn both into rejections that a synchronous
 * assertion cannot see. The guard test fails loudly if that ever changes, since a returned rejected
 * promise is not a throw.
 */
function runInstallHost(
  args: InstallArgs,
  directory: string,
  ...
  // Spawned rather than spawnSync'd because these runs are the file's whole cost: each one is a
  // real PowerShell process running the real installer, and a synchronous spawn blocks the event
  // loop, which leaves the concurrency the suite below declares with no await point to use.
  return new Promise((resolve) => {
    const child = spawn("powershell", ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", driverPath], {
      env: {
        ...process.env,
        HOME: profile,
        USERPROFILE: profile,
    ...
    // The installer prompts for the bot token with Read-Host when neither -BotToken nor
    // -BotTokenFile is given. A synchronous spawn handed the child an already-closed stdin, so such
    // a run failed at once; a spawned child is given a live pipe and would wait on it forever, and
    // the runner applies no per-test timeout, so the whole file would hang rather than fail.
    child.stdin.end();
    let stdout = "";
    let stderr = "";
    ...
    // failure to launch as a null status, and every caller here already asserts on the status, so
    // this resolves the same shape and the failure stays a readable assertion.
    child.on("error", (error) => resolve({ status: null, stdout, stderr: stderr + error.message }));
    child.on("close", (status) => resolve({ status, stdout, stderr }));
  });
}

// Each of these drives a real PowerShell process running the real installer, which is what makes
// this file the suite's longest pole. They share nothing: every one builds its own fixture tree and
// its own settings and state directories under the temp root, so they are safe to run at once. The
// bound is a number rather than `true` because the runner is already running other files alongside
// this one, and an unbounded fan-out would put every installer run and its icacls calls on the
// machine at the same moment.
describe("Install-Host end to end", { concurrency: 8 }, () => {
  it("the harness refuses to run an install that is not fenced into a fixture", () => {
  ...
  it("Install-Host provisions config, merges hooks, hardens the fixture tree, and creates the token file", async (t) => {
  ...
  it("Install-Host rejects a channel or user ID that is not a Discord snowflake", async (t) => {
    const repoRoot = fixtureRepoRoot();
    const settingsDir = mkdtempSync(path.join(os.tmpdir(), "channels-fixture-settings-"));
    const stateRoot = mkdtempSync(path.join(os.tmpdir(), "channels-fixture-state-"));
    t.after(() => {
  ...
  it("Install-Host refuses -SkipAcl combined with a token", async (t) => {
    const repoRoot = fixtureRepoRoot();
    const settingsDir = mkdtempSync(path.join(os.tmpdir(), "channels-fixture-settings-"));
    const stateRoot = mkdtempSync(path.join(os.tmpdir(), "channels-fixture-state-"));
    t.after(() => {
  ...
  it("Install-Host refuses a -Port that disagrees with the fragment's own literal", async (t) => {
    const repoRoot = fixtureRepoRoot();
    const settingsDir = mkdtempSync(path.join(os.tmpdir(), "channels-fixture-settings-"));
    const stateRoot = mkdtempSync(path.join(os.tmpdir(), "channels-fixture-state-"));
    t.after(() => {
  ...
  it("Install-Host refuses a fragment whose http hooks name more than one port", async (t) => {
  ...
  for (const { tree, victim, names } of HARDENED_TREES) {
    it(`Install-Host fails the install when a file under ${tree} is left open`, async (t) => {
  ...
  it("Install-Host verifies the token file, which is what covers the state root", async (t) => {
  ...
  it("Install-Host refuses a -BotTokenFile outside the state root", async (t) => {
    const repoRoot = fixtureRepoRoot();
    const settingsDir = mkdtempSync(path.join(os.tmpdir(), "channels-fixture-settings-"));
    const stateRoot = mkdtempSync(path.join(os.tmpdir(), "channels-fixture-state-"));
